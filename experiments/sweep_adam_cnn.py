...
def sweep_adam_on_cnn():
    ...
    print(f"Sweeping Adam LRs on Digits (100 steps)...")
    ...
    for seed in seeds:
        ...
        l_aa, _ = train_with_anderson(m2, X, y, loss_fn, steps=100, lr=0.1, momentum=0.7, window=5, aa_interval=10, safeguard=True)
    ...
    print(f"{'Seed':<6} | {'Best Adam (' + str(best_adam_lr) + ')':<20} | {'Baseline SGD-M (0.1)':<22} | {'Safeguarded Anderson':<22}")
    print("-" * 75)
    for i, seed in enumerate(seeds):
        print(f"{seed:<6} | {adam_results[best_adam_lr][i]:<20.6f} | {baseline_losses[i]:<22.6f} | {anderson_losses[i]:<22.6f}")
