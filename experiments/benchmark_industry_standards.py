...
def run_tinymlp_benchmark():
    ...
    optimizers_to_test = [
        ...
        ("NAG (Nesterov Accelerated Gradient)", lambda m: torch.optim.SGD(m.parameters(), lr=0.05, momentum=0.7, nesterov=True)),
    ...
    for name, losses in results.items():
        ...
        print(f"  * Paired t-test p-value:   {analysis['p_t']:.4e} {'(Statistically Significant)' if analysis['p_t'] < 0.05 else '(Not Sig)'}")
