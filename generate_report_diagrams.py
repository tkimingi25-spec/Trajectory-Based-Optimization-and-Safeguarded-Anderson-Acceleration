...
for i in range(len(x_pts)-1):
    axs[0].annotate('', xy=(x_pts[i+1], y_pts[i+1]), xytext=(x_pts[i], y_pts[i]),
                    arrowprops=dict(arrowstyle="->", color='darkred', lw=1.5))

# Anderson jump
x_jump = 1.3
y_jump = x_jump**4 - 3*x_jump**2 + 0.5*x_jump + 3
axs[0].annotate('Anderson Secant Jump\n(Cuts across ravine)', xy=(x_jump, y_jump), xytext=(-0.5, 7.5),
...
axs[0].set_title('(A) Smooth C^infinity Landscape (Tanh / GELU)\nSecant extrapolations predict continuous trajectory', fontsize=11, weight='bold', pad=10)
...
axs[1].annotate('Secant crosses kink\n(Invalid Linear Model)', xy=(1.0, 7.0), xytext=(-1.5, 9.0),
                arrowprops=dict(arrowstyle="->", color='red', lw=2, ls=':'),
                bbox=dict(boxstyle="round,pad=0.4", fc="#ffcccc", ec="red", alpha=0.8),
                fontsize=9, weight='bold')

axs[1].set_title('(B) Piecewise-Linear Landscape (ReLU)\nNon-smooth kinks invalidate secant differences', fontsize=11, weight='bold', pad=10)
...
ax.text(0.12, 0.75, "Step 1: State History\ns_t = [w_t; v_t] in R^(2D)\nStores (m+1) States", ha="center", va="center", bbox=box_blue, fontsize=9, weight="bold")
ax.text(0.40, 0.75, "Step 2: Type-II Least Squares\nmin ||F_diff * gamma - f_m||^2\nSolve m x m linear system", ha="center", va="center", bbox=box_gold, fontsize=9, weight="bold")
ax.text(0.72, 0.75, "Step 3: State Extrapolation\ns_jump = sum alpha_i * s_i\nJoint w & v update", ha="center", va="center", bbox=box_blue, fontsize=9, weight="bold")

# Decision Diamond / Box
ax.text(0.72, 0.35, "Step 4: Strict-Descent Safeguard\nIs Loss(s_jump) < Loss(s_pre)?", ha="center", va="center", bbox=box_gold, fontsize=9.5, weight="bold")

# Branches
ax.text(0.40, 0.15, "ACCEPT JUMP\nReset History & Accelerate\n(Loss cuts 40%, 60/60 wins)", ha="center", va="center", bbox=box_green, fontsize=9, weight="bold")
ax.text(0.92, 0.15, "REJECT JUMP\nRevert to s_pre\n(Zero penalty)", ha="center", va="center", bbox=box_red, fontsize=9, weight="bold")
